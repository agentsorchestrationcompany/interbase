...
import { createOpencode } from "@interbase/sdk"
...
`)
  ...
  const interbaseCli = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await interbaseCli.client.session.create()
    const result = await interbaseCli.client.session
      .prompt({
