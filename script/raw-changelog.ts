...
const repo = process.env.GH_REPO ?? "agentsorchestrationcompany/interbase"
const bot = ["actions-user", "github-actions[bot]", "interbase", "interbase-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/cli-host packages/core packages/runtime-protocol packages/sdk packages/plugin packages/cli-* github docs script`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/cli-host/src/cli/cmd/tui/")) areas.add("tui")
      else if (file.startsWith("packages/cli-host/") || file.startsWith("packages/core/")) areas.add("core")
      else if (file.startsWith("packages/sdk/") || file.startsWith("packages/plugin/")) areas.add("sdk")
      else if (file.startsWith("packages/runtime-protocol/") || file.startsWith("packages/cli-")) areas.add("sdk")
