...
Practical reference for extracting a future `packages/server` from the current `packages/cli-host` monolith while `packages/core` is still being migrated to Effect.
...
Everything still lives in `packages/cli-host`.
...
- the current server host still lives in `packages/cli-host`
...
- if `packages/server` imports services from `packages/cli-host` while `packages/cli-host` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/cli-host` as the temporary Hono host
...
- `packages/server` must not import from `packages/cli-host`

Allowed in phase 1:

- `packages/cli-host` imports `packages/server`
...
- `packages/cli-host` shrinks or disappears as package responsibilities are fully split
...
- `packages/cli-host` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/cli-host`
...
Keep in `packages/cli-host` for now:
...
- host mounting from `packages/cli-host`
...
- no imports from `packages/cli-host` inside `packages/server`
...
- `packages/server` must still not import from `packages/cli-host`
...
### PR 4. Mount `packages/server` question from `packages/cli-host`

Scope:

- replace local experimental question route wiring in `packages/cli-host`
...
- mount it in parallel from `packages/cli-host`
...
- move adapter selection and server startup out of `packages/cli-host`

Rules:

- do not start this while `packages/server` still depends on `packages/cli-host`
...
- host logic no longer lives in `packages/cli-host`
...
3. the route is mounted from the current host in `packages/cli-host`
...
- `packages/server` imports services or runtime from `packages/cli-host`
- `packages/cli-host` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/cli-host` imports
...
- [ ] mount `question` from `packages/cli-host`
...
2. keep `packages/cli-host` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/cli-host`, stop and restructure the boundary first.
