...
8. Run `bun typecheck` from `packages/cli-host`, relevant `bun run test:ci ...` tests from `packages/cli-host`, and `./packages/sdk/js/script/build.ts` from the repo root.
...
- Good first batch: `GET /console`, `GET /tool/ids`, `GET /resource`.
- Consider `GET /worktree` only if the handler uses `InstanceState.context` instead of `Instance.project`.
- Defer worktree create/remove/reset and `GET /session` to separate PRs because they mutate state or have broader pagination/session behavior.
