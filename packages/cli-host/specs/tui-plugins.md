...
- Author package entrypoint is `@interbase/plugin/tui`.
...
- Import types from `@interbase/plugin/tui`.
- `@interbase/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@interbase/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@interbase/plugin/tui`.
...
- `ui.Prompt` renders the same prompt component used by the host app and accepts `sessionID`, `ref`, and `right` for the prompt meta row's right side.
...
- `app`
- `home_logo`
- `home_prompt` with props `{ ref? }`
- `home_prompt_right` with props `{}`
