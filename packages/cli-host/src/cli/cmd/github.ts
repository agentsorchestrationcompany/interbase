...
const WORKFLOW_FILE = ".github/workflows/interbase.yml"
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://interbase.ai/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
...
jobs:
  interbase:
    if: |
      ...
      contains(github.event.comment.body, ' /interbase') ||
      startsWith(github.event.comment.body, '/interbase')
    ...
    steps:
      ...
      - name: Run interbase
        uses: agentsorchestrationcompany/interbase/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        appToken = args.token || process.env["GITHUB_TOKEN"] || ""
        if (!appToken) {
        ...
        console.log("interbase session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/interbase,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to interbase...")
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `interbase/${type}-${hex}-${timestamp}`
        }
        return `interbase/${type}${issueId}-${timestamp}`
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the interbase infrastructure after your response",
