...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@interbase/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run interbase with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running interbase server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to INTERBASE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to INTERBASE_SERVER_USERNAME or 'interbase')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://interbase.internal", fetch: fetchFn })
      await execute(sdk)
    })
  }),
})
