...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: interbase mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in interbase.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .interbase/ subdirectory too)
  const candidates = [path.join(baseDir, "interbase.json"), path.join(baseDir, "interbase.jsonc")]

  if (!global) {
    candidates.push(
      path.join(baseDir, ".interbase", "interbase.json"),
      path.join(baseDir, ".interbase", "interbase.jsonc"),
    )
  }
  ...
  // Default to interbase.json if none exist
  return candidates[0]
}
  ...
  handler: Effect.fn("Cli.mcp.add")(function* () {
    ...
    yield* Effect.promise(async () => {
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([
        ...
        resolveConfigPath(interbaseRuntimeContext.paths.config, true),
      ])
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., interbase x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "interbase-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
