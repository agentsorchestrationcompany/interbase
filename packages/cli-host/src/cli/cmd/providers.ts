...
import type { Hooks } from "@interbase/plugin"
import { Process } from "@/util/process"
import { errorMessage } from "@/util/error"
import { text } from "node:stream/consumers"
import { Effect, Option } from "effect"
import { resolvePluginProviderContributions } from "@interbase/plugin-provider-catalog"
  ...
  if (method.type === "oauth") {
    ...
    if (authorize.method === "auto") {
      ...
      if (result.type === "failed") {
        emitCliBehaviorTelemetry(CliTelemetryEvent.ProviderAuthFailed)
      ...
      if (result.type === "success") {
        ...
        emitCliBehaviorTelemetry(CliTelemetryEvent.ProviderAuthCompleted)
...
export function resolvePluginProviders(input: {
  ...
  return resolvePluginProviderContributions(input)
}
...
export const ProvidersListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.list")(function* (_args) {
    ...
    UI.empty()
    const authPath = path.join(interbaseRuntimeContext.paths.data, "auth.json")
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "interbase auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    emitCliBehaviorTelemetry(CliTelemetryEvent.ProviderAuthStarted)
    const authSvc = yield* Auth.Service
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/interbase`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in interbase.json, check the docs for examples.`,
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          "  1. Bearer token (AWS_BEARER_TOKEN_BEDROCK or /provider)\n" +
          "  2. AWS credential chain (profile, access keys, IAM roles, EKS IRSA)\n\n" +
          "Configure via interbase.json options (profile, region, endpoint) or\n" +
          "AWS environment variables (AWS_PROFILE, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_WEB_IDENTITY_TOKEN_FILE).",
      )
    }

    if (provider === "interbase") {
      yield* Prompt.log.info("Create an api key at https://interbase.ai/auth")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://interbase.ai/docs/providers/#cloudflare-ai-gateway",
      )
    }
