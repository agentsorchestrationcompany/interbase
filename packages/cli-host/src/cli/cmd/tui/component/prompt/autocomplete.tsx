...
export function Autocomplete(props: {
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    const handledSlashNames = new Set(
      results
    ...
    for (const serverCommand of sync.data.command) {
      ...
      if (!label && handledSlashNames.has(display)) continue
      results.push({
        display,
  ...
  function select() {
    ...
    const newText = slashAutocompleteInsertionText(selected, store.visible)
    if (newText) {
      const cursor = props.input().logicalCursor
      props.input().deleteRange(0, 0, cursor.row, cursor.col)
      props.input().insertText(newText)
  ...
  onMount(() => {
    ...
    props.ref({
      ...
      onInput(value) {
        if (store.visible) {
          if (store.visible === "/" && matchesExactSlashArgumentAlias(value, commands())) {
            hide({ preserveInput: true })
            return
          }
          if (
  ...
  const autocompleteBorderInset = createMemo(() => (store.visible ? 2 : 0))

  const height = createMemo(() => {
    const count = options().length || 1
    if (!store.visible) return Math.min(10, count)
    positionTick()
    return Math.min(10, count, Math.max(1, props.anchor().y - autocompleteBorderInset()))
  })
  ...
  return (
    <box
      ...
      position="absolute"
      top={position().y - height() - autocompleteBorderInset()}
      left={position().x}
