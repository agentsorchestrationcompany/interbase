...
export class Service extends Context.Service<Service, Interface>()("@interbase/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old interbase.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir and any `.interbase`
  // folders between cwd and home.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.INTERBASE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  for (const file of ConfigPaths.fileInDirectory(runtimeConfigDirectory, "tui")) {
    yield* mergeFile(acc, file)
  }

  // 2. Explicit INTERBASE_TUI_CONFIG override, if set.
  if (Flag.INTERBASE_TUI_CONFIG) {
    const configFile = Flag.INTERBASE_TUI_CONFIG
  ...
  // 4. `.interbase` directories and the runtime config directory discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".interbase") || dir === runtimeConfigDirectory)

  for (const dir of dirs) {
    if (!dir.endsWith(".interbase") && dir !== runtimeConfigDirectory) continue
