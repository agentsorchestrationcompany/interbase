...
const TUI_SCHEMA_URL = "https://interbase.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from interbase.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const interbase = await legacyConfigFiles(input)
  for (const file of interbase) {
...
async function legacyConfigFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(interbaseRuntimeContext.paths.config, "interbase"),
    ...(await Filesystem.findUp(["interbase.json", "interbase.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "interbase"))
  }
  if (Flag.INTERBASE_CONFIG) files.push(Flag.INTERBASE_CONFIG)
