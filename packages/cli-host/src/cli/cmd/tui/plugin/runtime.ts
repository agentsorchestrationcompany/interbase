...
import { INTERNAL_TUI_PLUGINS, type InternalTuiPlugin, type InternalTuiPluginRegistration } from "./internal"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".interbase"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".interbase", "themes")
    const dest_dir = meta.scope === "local" ? local_dir : path.join(interbaseRuntimeContext.paths.config, "themes")
...
async function loadInternalPlugin(item: InternalTuiPluginRegistration): Promise<PluginLoad> {
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".interbase", "tui.json"),
  }
}
...
export function hasSlot(name: string) {
  return runtime?.slots.has(name) ?? false
}
...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await WithInstance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.INTERBASE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.INTERBASE_PURE && config.plugin_origins?.length) {
          log.info("skipping external tui plugins in pure mode", { count: config.plugin_origins.length })
        }

        const internal = await Promise.all(
          INTERNAL_TUI_PLUGINS.map(async (item) => {
            log.info("loading internal tui plugin", { id: item.id })
            const entry = await loadInternalPlugin(item)
