...
import { PROVIDER_AUTH_TIP, PROVIDER_NO_MODELS_TIP, PROVIDER_ZEN_TIP } from "@interbase/plugin-provider-catalog"
...
export function Tips(props: { connected?: boolean }) {
  ...
  const parts = createMemo(() => parse(props.connected === false ? PROVIDER_NO_MODELS_TIP : randomTip))
...
const TIPS = [
  ...
  themeTip,
  "Press {highlight}Ctrl+X N{/highlight} or {highlight}/clear{/highlight} to start a fresh conversation session",
  ...
  "Press {highlight}Ctrl+O{/highlight} or {highlight}Ctrl+X Y{/highlight} to copy the assistant's last message to clipboard",
  ...
  "Press {highlight}Ctrl+S{/highlight} to show/hide the sidebar panel",
  ...
  "Create {highlight}interbase.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/interbase/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Interbase auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.interbase/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.interbase/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Interbase auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Interbase uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.interbase/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.interbase/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Interbase from reading sensitive files",
  "Use {highlight}interbase run{/highlight} for non-interactive scripting",
  "Use {highlight}interbase --continue{/highlight} to resume the last session",
  "Use {highlight}interbase run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}interbase serve{/highlight} for headless API access to Interbase",
  "Use {highlight}interbase run --attach{/highlight} to connect to a running server",
  "Run {highlight}interbase upgrade{/highlight} to update to the latest version",
  "Run {highlight}interbase auth list{/highlight} to see all configured providers",
  "Run {highlight}interbase agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.interbase/themes/{/highlight} directory",
  ...
  "Run {highlight}interbase debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/agentsorchestrationcompany/interbase{/highlight} for containerized use",
  PROVIDER_ZEN_TIP,
