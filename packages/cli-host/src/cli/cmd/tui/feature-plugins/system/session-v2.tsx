...
function CompactionMessage(props: { message: SessionMessageCompaction }) {
  ...
  return (
    <box
      marginTop={1}
      border={["top"]}
      title={props.message.phase === "manual" ? " Compaction " : " Auto Compaction "}
  ...
  return (
    <box marginTop={1} paddingLeft={3} flexShrink={0}>
      <Switch>
        ...
        <Match when={true}>
          <Show fallback={<Spinner color={theme.text}>{props.pending}</Spinner>} when={complete()}>
      ...
      <Match when={diff()}>
        {(diff) => (
          <HunkedDiffBlock
            ...
            footer={<Diagnostics diagnostics={props.metadata.diagnostics} filePath={filePath()} />}
            footerWhen={hasDiagnosticErrors(props.metadata.diagnostics, filePath())}
              diff={diff()}
...
function hasDiagnosticErrors(diagnostics: unknown, filePath: string) {
  if (!isRecord(diagnostics)) return false
  const value = diagnostics[normalizePath(filePath)] ?? diagnostics[filePath]
  return arrayValue(value).some((item) => isRecord(item) && item.severity === 1)
}
