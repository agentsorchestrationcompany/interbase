...
import { Glob } from "@interbase/core/util/glob"
...
import legacyTheme from "./theme/opencode.json" with { type: "json" }
import osakaJade from "./theme/osaka-jade.json" with { type: "json" }
import onedark from "./theme/one-dark.json" with { type: "json" }
import interbase from "@interbase/overlay/theme/interbase.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@interbase/plugin/tui"
import { resolveThemeMode, type SystemThemeObserver, type ThemeMode, type ThemeScheme } from "@/util/system-theme"
...
type SelectionColorRenderer = {
  setSelectionColors?: (selectionBg: RGBA | undefined, selectionFg: RGBA | undefined) => void
  selectionBg?: RGBA
  selectionFg?: RGBA
}
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  name: "Theme",
  init: (props: { mode: ThemeMode; systemTheme: SystemThemeObserver }) => {
    ...
    const pickScheme = (value: unknown) => {
    ...
    const autoMode = () =>
      resolveThemeMode({
        scheme: "system",
        system: props.systemTheme.current(),
        renderer: pickMode(renderer.themeMode),
        fallback: props.mode,
      })

    setStore(
      produce((draft) => {
        const legacyMode = pickMode(kv.get(INTERBASE_THEME_SCHEME.legacyModeKey))
        const legacyScheme = pickMode(kv.get(INTERBASE_THEME_SCHEME.legacyLockKey))
        const savedScheme =
          pickScheme(kv.get(INTERBASE_THEME_SCHEME.storageKey)) ?? legacyScheme ?? INTERBASE_THEME_SCHEME.defaultScheme
        ...
        if (legacyMode !== undefined || legacyScheme !== undefined) {
          kv.set(INTERBASE_THEME_SCHEME.legacyModeKey, undefined)
          kv.set(INTERBASE_THEME_SCHEME.legacyLockKey, undefined)
        }
        if (pickScheme(kv.get(INTERBASE_THEME_SCHEME.storageKey)) === undefined) {
          kv.set(INTERBASE_THEME_SCHEME.storageKey, savedScheme)
        }
        draft.mode = mode
        draft.scheme = savedScheme
        const active = kv.get("theme") ?? config.theme ?? "interbase"
        draft.active = typeof active === "string" ? active : "interbase"
        draft.ready = false
      }),
    )
    ...
    function resolveSystemTheme(mode: ThemeMode = store.mode) {
      return renderer
        .getPalette({
          size: 16,
        })
    ...
    function applyScheme(scheme: ThemeScheme) {
      ...
      kv.set(INTERBASE_THEME_SCHEME.storageKey, scheme)
      apply(
        resolveThemeMode({
          scheme,
          system: props.systemTheme.current(),
    ...
    const handle = (mode: ThemeMode) => {
      if (store.scheme !== "system" || props.systemTheme.available) return
      apply(mode)
    }
    renderer.on(CliRenderEvents.THEME_MODE, handle)
    const stopSystemTheme = props.systemTheme.observe((mode) => {
      if (store.scheme !== "system") return
      apply(mode)
    })
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.interbase, store.mode)
    })

    createEffect(() => {
      const theme = values()
      const selectionBg = theme.primary
      const selectionFg = selectedForeground(theme, selectionBg)
      const selectableRenderer = renderer as typeof renderer & SelectionColorRenderer
      renderer.setBackgroundColor(theme.background)
      if (selectableRenderer.setSelectionColors) {
        selectableRenderer.setSelectionColors(selectionBg, selectionFg)
      } else {
        selectableRenderer.selectionBg = selectionBg
        selectableRenderer.selectionFg = selectionFg
      }
    })
    ...
    return {
      ...
      set(theme: string) {
        ...
        if (store.active !== theme) emitCliBehaviorTelemetry(CliTelemetryEvent.ThemeChanged, CliTelemetryEntrypoint.Tui)
...
async function getCustomThemes() {
  const directories = [
    interbaseRuntimeContext.paths.config,
