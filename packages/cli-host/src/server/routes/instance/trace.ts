...
// `interbase.<name>` since those are internal implementation details
...
export function createRunRequest(runtime: AppRuntimeLike = AppRuntime) {
  return <A, E>(name: string, c: Context, effect: Effect.Effect<A, E, AppEnv>) =>
    runtime.runPromise(effect.pipe(Effect.withSpan(name, { attributes: requestAttributes(c) })))
}
...
export function createJsonRequest(runtime: AppRuntimeLike = AppRuntime) {
  const run = createRunRequest(runtime)
  return async <C extends Context, A, E>(name: string, c: C, effect: (c: C) => Effect.gen.Return<A, E, AppEnv>) =>
    c.json(
      await run(
        name,
        c,
