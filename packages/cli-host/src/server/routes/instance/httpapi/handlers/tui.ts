...
const telemetryEventsByCommand = {
  session_compact: CliTelemetryEvent.SessionCompacted,
  session_interrupt: CliTelemetryEvent.SessionInterrupted,
  session_new: CliTelemetryEvent.SessionNewRequested,
} as const
...
export const tuiHandlers = HttpApiBuilder.group(InstanceHttpApi, "tui", (handlers) =>
  Effect.gen(function* () {
    ...
    const openSessions = Effect.fn("TuiHttpApi.openSessions")(function* () {
      emitCliBehaviorTelemetry(CliTelemetryEvent.SessionPickerOpened, CliTelemetryEntrypoint.Tui)
      yield* publishCommand("session.list")
      return true
    })

    const openThemes = Effect.fn("TuiHttpApi.openThemes")(function* () {
      emitCliBehaviorTelemetry(CliTelemetryEvent.ThemePickerOpened, CliTelemetryEntrypoint.Tui)
      yield* publishCommand("session.list")
      return true
    })

    const openModels = Effect.fn("TuiHttpApi.openModels")(function* () {
      emitCliBehaviorTelemetry(CliTelemetryEvent.ModelPickerOpened, CliTelemetryEntrypoint.Tui)
      yield* publishCommand("model.list")
      return true
    })

    const submitPrompt = Effect.fn("TuiHttpApi.submitPrompt")(function* () {
      emitCliBehaviorTelemetry(CliTelemetryEvent.PromptSubmitted, CliTelemetryEntrypoint.Tui)
    ...
    const executeCommand = Effect.fn("TuiHttpApi.executeCommand")(function* (ctx: {
      ...
      const event = telemetryEventsByCommand[ctx.payload.command as keyof typeof telemetryEventsByCommand]
      if (event) emitCliBehaviorTelemetry(event, CliTelemetryEntrypoint.Tui)
    ...
    const selectSession = Effect.fn("TuiHttpApi.selectSession")(function* (ctx: {
      ...
      emitCliBehaviorTelemetry(CliTelemetryEvent.SessionResumed, CliTelemetryEntrypoint.Tui)
