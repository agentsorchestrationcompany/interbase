  ...
  Effect.gen(function* () {
    ...
    const update = Effect.fn("SessionHttpApi.update")(function* (ctx: {
      ...
      if (ctx.payload.model !== undefined) {
        const model = yield* provider.getModel(ctx.payload.model.providerID, ctx.payload.model.id).pipe(
          Effect.catchIf(Provider.ModelNotFoundError.isInstance, () => Effect.fail(new HttpApiError.BadRequest({}))),
          Effect.catchDefect((error) =>
