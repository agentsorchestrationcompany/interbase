...
// Instance routes use middleware for directory resolution, but HttpApi doesn't
// surface middleware query params in the spec. Inject it explicitly.
...
const LegacyComponentDescriptions = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the interbase serve command",
  ...
  for (const [path, item] of Object.entries(spec.paths ?? {})) {
    ...
    for (const method of ["get", "post", "put", "delete", "patch"] as const) {
      ...
      operation.parameters = [
        ...
        ...(operation.parameters ?? []).filter((param) => param.in !== "query" || param.name !== "directory"),
      ]
...
function createLegacyOpenApiTransform(bundles: readonly FeatureBundle[]) {
  void bundles
  return (input: OpenApiSpec) => {
    return matchLegacyOpenApi(input)
  }
}

export function createPublicApi(_bundles: readonly FeatureBundle[] = ACTIVE_FEATURE_BUNDLES) {
  return OpenCodeHttpApi.annotateMerge(
    OpenApi.annotations({
      title: "interbase",
      version: "1.0.0",
      description: "interbase api",
      transform: createLegacyOpenApiTransform(_bundles),
    }),
  )
}
