...
const MANAGED_PLIST_DOMAIN = "ai.interbase.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/interbase"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "interbase")
    default:
      return "/etc/interbase"
  }
}

export function managedConfigDir() {
  return process.env.INTERBASE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
