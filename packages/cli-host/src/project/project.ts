...
export interface Interface {
  /**
   * Per-instance setup for project-scoped background work.
   */
...
export class Service extends Context.Service<Service, Interface>()("@interbase/Project") {}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.INTERBASE_FAKE_VCS)
    ...
    const readCachedProjectId = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(pathSvc.join(dir, "interbase")).pipe(
        Effect.map((x) => x.trim()),
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        if (!id) {
          ...
          if (id) {
            yield* fs.writeFileString(pathSvc.join(common, "interbase"), id).pipe(Effect.ignore)
          }
        }
      ...
      if (Flag.INTERBASE_EXPERIMENTAL_ICON_DISCOVERY)
