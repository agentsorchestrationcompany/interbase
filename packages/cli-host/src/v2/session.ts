...
export class Service extends Context.Service<Service, Interface>()("@interbase/v2/Session") {}
    ...
    const result: Interface = {
      list: Effect.fn("V2Session.list")(function* (input) {
        ...
        const rows = Database.listPaths()
          .flatMap((dbPath) => {
            const query = Database.Client(dbPath)
              .select()
              .from(SessionTable)
          ...
          .sort((a, b) =>
            order === "asc"
              ...
              : b.time_created - a.time_created || b.id.localeCompare(a.id),
          )
          .filter((row, index, items) => items.findIndex((item) => item.id === row.id) === index)
        const sliced = input.limit === undefined ? rows : rows.slice(0, input.limit)
        return (direction === "previous" ? sliced.toReversed() : sliced).map((row) => fromRow(row))
      }),
      ...
      context: Effect.fn("V2Session.context")(function* (sessionID) {
        const rows = Database.usePath(Database.resolveSessionPath(sessionID) ?? Database.Path, (db) => {
          const compaction = db
            .select()
            .from(SessionMessageTable)
