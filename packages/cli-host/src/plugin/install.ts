...
export type PatchDeps = {
  ...
  files: (dir: string, name: "interbase" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  if (input.global) return input.config ?? interbaseRuntimeContext.paths.config
  const git = input.vcs === "git" && input.worktree !== "/"
  const root = git ? input.worktree : input.directory
  return path.join(root, ".interbase")
}

function patchName(kind: Kind): "interbase" | "tui" {
  if (kind === "server") return "interbase"
  return "tui"
}
