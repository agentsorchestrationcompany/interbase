import type {
  ...
  PluginModule,
} from "@interbase/plugin"
import { Config } from "@/config/config"
import { Bus } from "../bus"
import * as Log from "@interbase/core/util/log"
import { createOpencodeClient } from "@interbase/sdk"
...
export class Service extends Context.Service<Service, Interface>()("@interbase/Plugin") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Plugin.state")(function* (ctx) {
        ...
        for (const registration of requiredInternalPlugins()) {
          ...
          const init = yield* Effect.tryPromise({
            try: () => plugin(input),
              ...
              log.error("failed to load internal plugin", { id: registration.id, name: plugin.name, error: err })
            },
          }).pipe(Effect.option)
          if (init._tag === "Some") hooks.push(init.value)
        }

        const plugins = Flag.INTERBASE_PURE ? [] : (cfg.plugin_origins ?? [])
        if (Flag.INTERBASE_PURE && cfg.plugin_origins?.length) {
        ...
        yield* bus.subscribeAll().pipe(
          Stream.runForEach((input) =>
            Effect.promise(async () => {
              for (const hook of hooks) {
                await hook["event"]?.({ event: input as any })
              }
            }),
          ),
          Effect.forkScoped,
        ...
        const onGlobalEvent = (event: { directory?: string; payload: any }) => {
          ...
          if (!event.payload?.event) return
          if (event.directory !== ctx.directory) return
          for (const hook of hooks) {
            void hook["event"]?.({ event: event.payload.event })
          }
        }
