...
export const DEPRECATED_PLUGIN_PACKAGES = ["interbase-openai-codex-auth", "interbase-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, interbaseVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(interbaseVersion) || semver.major(interbaseVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.interbase
  if (typeof range !== "string") return
  if (!semver.satisfies(interbaseVersion, range)) {
    throw new Error(`Plugin requires interbase ${range} but running ${interbaseVersion}`)
  }
}
