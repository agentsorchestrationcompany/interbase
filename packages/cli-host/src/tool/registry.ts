...
import { type ToolContext as PluginToolContext, type ToolDefinition } from "@interbase/plugin"
...
export class Service extends Context.Service<Service, Interface>()("@interbase/ToolRegistry") {}
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("ToolRegistry.state")(function* (ctx) {
        ...
        const questionEnabled =
          ["app", "cli", "desktop"].includes(Flag.INTERBASE_CLIENT) || Flag.INTERBASE_ENABLE_QUESTION_TOOL
        ...
        return {
          ...
          builtin: [
            ...
            tool.skill,
            tool.patch,
            ...(Flag.INTERBASE_EXPERIMENTAL_LSP_TOOL ? [tool.lsp] : []),
            ...(Flag.INTERBASE_EXPERIMENTAL_PLAN_MODE && Flag.INTERBASE_CLIENT === "cli" ? [tool.plan] : []),
          ],
          task: tool.task,
          read: tool.read,
        }
    ...
    const tools: Interface["tools"] = Effect.fn("ToolRegistry.tools")(function* (input) {
      const filtered = (yield* all()).filter((tool) => {
        if (tool.id === WebSearchTool.id) {
          return input.providerID === ProviderID.interbase || Flag.INTERBASE_ENABLE_EXA
        }
