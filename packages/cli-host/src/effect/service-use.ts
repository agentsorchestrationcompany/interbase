  ...
  const access = new Proxy(
    {},
    {
      get: (_, key) => {
        ...
        return (...args: unknown[]) =>
          tag.use((service) => {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- Proxy keys are checked at runtime.
            ...
            // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- ServiceUse exposes only Effect-returning methods.
            return (method as (...args: unknown[]) => Effect.Effect<unknown, unknown, unknown>)(...args)
          })
      },
    },
  )
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- Proxy implements the mapped accessor surface lazily.
