import { Slug } from "@interbase/core/util/slug"
...
const sessionPathFor = (sessionID: SessionID) => Database.resolveSessionPath(sessionID) ?? Database.Path

const parentTitlePrefix = "New session - "
const childTitlePrefix = "Child session - "

function formatModelLabel(model: { providerID: string; id: string; variant?: string }) {
...
export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".interbase", "plans")
    : path.join(interbaseRuntimeContext.paths.data, "plans")
...
export class Service extends Context.Service<Service, Interface>()("@interbase/Session") {}
...
export const layer: Layer.Layer<Service, never, Bus.Service | Storage.Service | SyncEvent.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      yield* sync.run(Event.Created, { sessionID: result.id, info: result }, { path: input.dbPath })

      yield* bus.publish(Event.Updated, {
        sessionID: result.id,
        info: result,
      })
      emitCliBehaviorTelemetry(CliTelemetryEvent.SessionCreated)
    ...
    const get = Effect.fn("Session.get")(function* (id: SessionID) {
      const row = yield* Effect.sync(() =>
        Database.usePath(sessionPathFor(id), (d) => d.select().from(SessionTable).where(eq(SessionTable.id, id)).get()),
      )
    ...
    const list = Effect.fn("Session.list")(function* (input?: ListInput) {
      const ctx = yield* InstanceState.context
      return listByProject({ projectID: ctx.project.id, ...(input ?? {}) })
    })

    const children = Effect.fn("Session.children")(function* (parentID: SessionID) {
      const rows = yield* Effect.sync(() =>
        Database.usePath(sessionPathFor(parentID), (d) =>
          d
            .select()
            .from(SessionTable)
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      try {
        ...
        const dbPath = sessionPathFor(sessionID)
        yield* sync.run(Event.Deleted, { sessionID, info: session }, { publish: hasInstance, path: dbPath })
        yield* sync.remove(sessionID, { path: dbPath })
      } catch (e) {
        log.error(e)
      }
    })
    ...
    const updateMessage = <T extends MessageV2.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        yield* sync.run(
          ...
          { path: sessionPathFor(msg.sessionID) },
        )
        return msg
    ...
    const updatePart = <T extends MessageV2.Part>(part: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        yield* sync.run(
          ...
          { path: sessionPathFor(part.sessionID) },
        )
        return part
      }).pipe(Effect.withSpan("Session.updatePart"))

    const getPart: Interface["getPart"] = Effect.fn("Session.getPart")(function* (input) {
      const row = Database.usePath(sessionPathFor(input.sessionID), (db) =>
        db
          .select()
          .from(PartTable)
          .where(
    ...
    const fork = Effect.fn("Session.fork")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      const ctx = yield* InstanceState.context
      const dbPath = sessionPathFor(input.sessionID)
      ...
      emitCliBehaviorTelemetry(CliTelemetryEvent.SessionForked)
      return session
    })

    const patch = (sessionID: SessionID, info: Patch) =>
      sync.run(Event.Updated, { sessionID, info }, { path: sessionPathFor(sessionID) })
    ...
    const setModel = Effect.fn("Session.setModel")(function* (input: { sessionID: SessionID; model: Info["model"] }) {
      if (!input.model) {
        yield* patch(input.sessionID, { model: null, time: { updated: Date.now() } })
        return
      }
      ...
      if (
        ...
        current.model.variant === input.model.variant
      ) {
        return
      }
      emitCliBehaviorTelemetry(CliTelemetryEvent.ModelSelected)
      ...
      const switchedModel = formatModelLabel(input.model)
      const recent = yield* messages({ sessionID: input.sessionID, limit: 1 })
      const previousModelSwitch = recent.find((message) => {
      ...
      const messageID = previousModelSwitch?.info.id ?? MessageID.ascending()
      const partID = previousModelSwitch?.parts[0]?.id ?? PartID.ascending()
      yield* updateMessage({
        id: messageID,
        sessionID: input.sessionID,
        role: "user",
        time: previousModelSwitch?.info.time ?? { created: Date.now() },
      ...
      yield* updatePart({
        ...
        messageID,
        type: "text",
        text: `Switched model to ${switchedModel}`,
        synthetic: true,
        ignored: true,
        metadata: {
      ...
      yield* sync.run(
        ...
        { path: sessionPathFor(input.sessionID) },
      )
    })
...
function listByProject(
  ...
  return (input.currentDatabaseOnly ? [Database.Path] : Database.listPaths())
    .flatMap((dbPath) =>
      Database.usePath(dbPath, (db) =>
        db
          .select()
          .from(SessionTable)
    ...
    .sort((a, b) => b.time_updated - a.time_updated || b.id.localeCompare(a.id))
...
export function* listGlobal(input?: {
  ...
  const rows = Database.listPaths()
    .flatMap((dbPath) => {
      const rows = Database.usePath(dbPath, (db) => {
        const query =
          conditions.length > 0
            ? db
                .select()
      ...
      if (ids.length > 0) {
        for (const item of Database.usePath(dbPath, (db) =>
          db
      ...
      return rows.map((row) => ({ row, project: projects.get(row.project_id) ?? null }))
    })
    .sort((a, b) => b.row.time_updated - a.row.time_updated || b.row.id.localeCompare(a.row.id))
    .filter((item, index, rows) => rows.findIndex((row) => row.row.id === item.row.id) === index)
    .slice(0, limit)
