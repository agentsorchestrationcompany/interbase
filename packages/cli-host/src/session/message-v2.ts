...
import { preservesProviderArtifacts, targetIdentity } from "@interbase/cli-model-switching"
...
export const CompactionPart = Schema.Struct({
  ...
  auto: Schema.Boolean,
  phase: Schema.optional(Schema.Literals(["manual", "preTurn", "midTurn"])),
  reason: Schema.optional(Schema.Literals(["contextLimit", "modelDownshift", "userRequested", "overflow"])),
  resume_model: Schema.optional(
    Schema.Struct({
...
export const Assistant = Schema.Struct({
  ...
  apiPackage: Schema.optional(Schema.String),
  modelID: ModelID,
  providerID: ProviderID,
  /**
   * @deprecated
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  options?: { stripMedia?: boolean; targetVariant?: string; toolOutputMaxChars?: number },
) {
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      const preserveProviderArtifacts = preservesProviderArtifacts({
        origin: {
          apiID: msg.info.apiID,
          apiPackage: msg.info.apiPackage,
      ...
      for (const part of msg.parts) {
        if (part.type === "text")
          assistantMessage.parts.push({
            type: "text",
            text: part.text,
            ...(preserveProviderArtifacts ? { providerMetadata: part.metadata } : {}),
          })
        ...
        if (part.type === "tool") {
          ...
          if (part.state.status === "completed") {
            ...
            assistantMessage.parts.push({
              ...
              ...(preserveProviderArtifacts ? { callProviderMetadata: providerMeta(part.metadata) } : {}),
            })
          }
...
export function page(input: { sessionID: SessionID; limit: number; before?: string }) {
  const dbPath = Database.resolveSessionPath(input.sessionID) ?? Database.Path
  ...
  if (rows.length === 0) {
    const row = Database.usePath(dbPath, (db) =>
...
export function get(input: { sessionID: SessionID; messageID: MessageID }): WithParts {
  ...
  const parts = Database.usePath(dbPath, (db) =>
    db.select().from(PartTable).where(eq(PartTable.message_id, input.messageID)).orderBy(PartTable.id).all(),
  )
  return {
    info: info(row),
    parts: parts.map(
      ...
      return new APIError(
        {
          ...
          ...(parsed.usageLimited === undefined ? {} : { usageLimited: parsed.usageLimited }),
...
export function isUsageLimitError(error: unknown): boolean {
  return (
    ...
    (error as { data?: { usageLimited?: unknown } }).data?.usageLimited === true
  )
}
