...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@interbase/sdk/v2"
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `interbase auth login` in the terminal",
      name: "Login with interbase",
      id: "interbase-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // Interbase stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const interbaseProvider = providers.find((provider) => provider.id === "interbase")
  if (interbaseProvider) {
    if (interbaseProvider.models["big-pickle"]) {
      return { providerID: ProviderID.interbase, modelID: ModelID.make("big-pickle") }
    }
    const [best] = Provider.sort(Object.values(interbaseProvider.models))
    if (best) {
      return {
