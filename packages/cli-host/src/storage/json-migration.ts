...
export async function run(db: SQLiteBunDatabase<any, any> | NodeSQLiteDatabase<any, any>, options?: Options) {
  const storageDir = path.join(interbaseRuntimeContext.paths.data, "storage")
  ...
  async function read(files: string[], start: number, end: number) {
    ...
    // eslint-disable-next-line unicorn/no-new-array -- pre-allocated for index-based batch fill
