...
import { sanitizedProcessEnv } from "@interbase/core/util/interbase-process"
...
export class Service extends Context.Service<Service, Interface>()("@interbase/Ripgrep") {}
    ...
    Effect.gen(function* () {
      ...
      const extract = Effect.fnUntraced(function* (
        ...
        const dir = yield* fs.makeTempDirectoryScoped({
          directory: interbaseRuntimeContext.paths.bin,
          prefix: "ripgrep-",
        })
      ...
      const filepath = yield* Effect.cached(
        Effect.gen(function* () {
          ...
          const target = path.join(interbaseRuntimeContext.paths.bin, `rg${process.platform === "win32" ? ".exe" : ""}`)
          ...
          const archive = path.join(interbaseRuntimeContext.paths.bin, filename)

          log.info("downloading ripgrep", { url })
          yield* fs.ensureDir(interbaseRuntimeContext.paths.bin).pipe(Effect.orDie)
      ...
      const tree: Interface["tree"] = Effect.fn("Ripgrep.tree")(function* (input: TreeInput) {
        ...
        for (const file of list) {
          if (file.includes(".interbase")) continue
