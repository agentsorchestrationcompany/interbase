...
export interface Interface {
  readonly run: <Def extends Definition>(
    def: Def,
    data: Event<Def>["data"],
    options?: { publish?: boolean; path?: string },
  ) => Effect.Effect<void>
  readonly replay: (event: SerializedEvent, options?: { publish: boolean; path?: string }) => Effect.Effect<void>
  readonly replayAll: (events: SerializedEvent[], options?: { publish: boolean }) => Effect.Effect<string | undefined>
  readonly remove: (aggregateID: string, options?: { path?: string }) => Effect.Effect<void>
}

export class Service extends Context.Service<Service, Interface>()("@interbase/SyncEvent") {}
  ...
  Effect.gen(function* () {
    const replay: Interface["replay"] = Effect.fn("SyncEvent.replay")(function* (event, options) {
      ...
      const context = publish
        ...
        : undefined
      process(def, event, { publish, context, path: options?.path ?? Database.resolveSessionPath(event.aggregateID) })
    })
    ...
    const run: Interface["run"] = Effect.fn("SyncEvent.run")(function* (def, data, options) {
      ...
      const { publish = true, path } = options || {}
      const context = publish
        ? {
            instance: yield* InstanceState.context,
          }
      ...
      Database.transaction(
        (tx) => {
          ...
          process(def, event, { publish, context, path: path ?? Database.resolveSessionPath(agg) })
        },
        {
          behavior: "immediate",
        },
    ...
    const remove: Interface["remove"] = Effect.fn("SyncEvent.remove")(function* (aggregateID, options) {
      Database.transactionPath(options?.path ?? Database.resolveSessionPath(aggregateID) ?? Database.Path, (tx) => {
...
function process<Def extends Definition>(
  def: Def,
  event: Event<Def>,
  options: { publish: boolean; context?: PublishContext; path?: string },
) {
  if (projectors == null) {
  ...
  Database.transactionPath(options.path ?? Database.resolveSessionPath(event.aggregateID) ?? Database.Path, (tx) => {
    ...
    Database.effect(() => {
      if (options?.publish) {
        ...
        const publish = (data: unknown) => {
          void ProjectBus.publish(def, data as Properties<Def>, { id: event.id })
...
export function remove(aggregateID: string, options?: { path?: string }) {
  return runtime.runSync((sync) => sync.remove(aggregateID, options))
}
