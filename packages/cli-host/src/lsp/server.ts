...
import { Module } from "@interbase/core/util/module"
...
export const Vue: Info = {
  ...
  async spawn(root) {
    ...
    const args: string[] = []
    if (!binary) {
      if (Flag.INTERBASE_DISABLE_LSP_DOWNLOAD) return
...
export const ESLint: Info = {
  ...
  async spawn(root, ctx) {
    ...
    const serverPath = path.join(interbaseRuntimeContext.paths.bin, "vscode-eslint", "server", "out", "eslintServer.js")
    if (!(await Filesystem.exists(serverPath))) {
      ...
      const zipPath = path.join(interbaseRuntimeContext.paths.bin, "vscode-eslint.zip")
      if (response.body) await Filesystem.writeStream(zipPath, response.body)

      const ok = await Archive.extractZip(zipPath, interbaseRuntimeContext.paths.bin)
        .then(() => true)
        .catch((error) => {
      ...
      const extractedPath = path.join(interbaseRuntimeContext.paths.bin, "vscode-eslint-main")
      const finalPath = path.join(interbaseRuntimeContext.paths.bin, "vscode-eslint")
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      const proc = Process.spawn(["go", "install", "golang.org/x/tools/gopls@latest"], {
        env: { ...process.env, GOBIN: interbaseRuntimeContext.paths.bin },
        stdout: "pipe",
        stderr: "pipe",
        stdin: "pipe",
      })
      ...
      bin = path.join(interbaseRuntimeContext.paths.bin, "gopls" + (process.platform === "win32" ? ".exe" : ""))
...
export const Rubocop: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      const proc = Process.spawn(["gem", "install", "rubocop", "--bindir", interbaseRuntimeContext.paths.bin], {
        stdout: "pipe",
        stderr: "pipe",
        stdin: "pipe",
      })
      ...
      bin = path.join(interbaseRuntimeContext.paths.bin, "rubocop" + (process.platform === "win32" ? ".exe" : ""))
...
export const Ty: Info = {
  ...
  async spawn(root) {
    if (!Flag.INTERBASE_EXPERIMENTAL_LSP_TY) {
      return undefined
    }
...
export const ElixirLS: Info = {
  ...
  async spawn(root) {
    ...
    if (!binary) {
      const elixirLsPath = path.join(interbaseRuntimeContext.paths.bin, "elixir-ls")
      binary = path.join(
        interbaseRuntimeContext.paths.bin,
        "elixir-ls-master",
        "release",
      ...
      if (!(await Filesystem.exists(binary))) {
        ...
        if (!response.ok) return
        const zipPath = path.join(interbaseRuntimeContext.paths.bin, "elixir-ls.zip")
        if (response.body) await Filesystem.writeStream(zipPath, response.body)
        ...
        const cwd = path.join(interbaseRuntimeContext.paths.bin, "elixir-ls-master")
...
export const Zls: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      const tempPath = path.join(interbaseRuntimeContext.paths.bin, assetName)
      if (downloadResponse.body) await Filesystem.writeStream(tempPath, downloadResponse.body)

      if (ext === "zip") {
        const ok = await Archive.extractZip(tempPath, interbaseRuntimeContext.paths.bin)
          .then(() => true)
          .catch((error) => {
        ...
        await run(["tar", "-xf", tempPath], { cwd: interbaseRuntimeContext.paths.bin })
      }

      await fs.rm(tempPath, { force: true })

      bin = path.join(interbaseRuntimeContext.paths.bin, "zls" + (platform === "win32" ? ".exe" : ""))
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      const proc = Process.spawn(
        ["dotnet", "tool", "install", "fsautocomplete", "--tool-path", interbaseRuntimeContext.paths.bin],
        {
          stdout: "pipe",
          stderr: "pipe",
          stdin: "pipe",
...
export const Clangd: Info = {
  ...
  async spawn(root) {
    ...
    const direct = path.join(interbaseRuntimeContext.paths.bin, "clangd" + ext)
    ...
    const entries = await fs.readdir(interbaseRuntimeContext.paths.bin, { withFileTypes: true }).catch(() => [])
    for (const entry of entries) {
      if (!entry.isDirectory()) continue
      if (!entry.name.startsWith("clangd_")) continue
      const candidate = path.join(interbaseRuntimeContext.paths.bin, entry.name, "bin", "clangd" + ext)
    ...
    const archive = path.join(interbaseRuntimeContext.paths.bin, name)
    ...
    if (zip) {
      const ok = await Archive.extractZip(archive, interbaseRuntimeContext.paths.bin)
        .then(() => true)
        .catch((error) => {
    ...
    if (tar) {
      await run(["tar", "-xf", archive], { cwd: interbaseRuntimeContext.paths.bin })
    }
    await fs.rm(archive, { force: true })

    const bin = path.join(interbaseRuntimeContext.paths.bin, "clangd_" + tag, "bin", "clangd" + ext)
    if (!(await Filesystem.exists(bin))) {
    ...
    await fs.unlink(path.join(interbaseRuntimeContext.paths.bin, "clangd")).catch(() => {})
    await fs.symlink(bin, path.join(interbaseRuntimeContext.paths.bin, "clangd")).catch(() => {})
...
export const JDTLS: Info = {
  ...
  async spawn(root) {
    ...
    const distPath = path.join(interbaseRuntimeContext.paths.bin, "jdtls")
    ...
    const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), "interbase-jdtls-data"))
    return {
      process: spawn(
        java,
        [
...
export const KotlinLS: Info = {
  ...
  async spawn(root) {
    const distPath = path.join(interbaseRuntimeContext.paths.bin, "kotlin-ls")
...
export const TerraformLS: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      const tempPath = path.join(interbaseRuntimeContext.paths.bin, "terraform-ls.zip")
      if (downloadResponse.body) await Filesystem.writeStream(tempPath, downloadResponse.body)
      ...
      bin = path.join(interbaseRuntimeContext.paths.bin, "terraform-ls" + (platform === "win32" ? ".exe" : ""))
...
export const TexLab: Info = {
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (ext === "tar.gz") {
        await run(["tar", "-xzf", tempPath], { cwd: interbaseRuntimeContext.paths.bin })
      }

      await fs.rm(tempPath, { force: true })

      bin = path.join(interbaseRuntimeContext.paths.bin, "texlab" + (platform === "win32" ? ".exe" : ""))
  ...
  async spawn(root) {
    ...
    if (!bin) {
      ...
      if (ext === "zip") {
        ...
        await run(["tar", "-xzf", tempPath, "--strip-components=1"], { cwd: interbaseRuntimeContext.paths.bin })
      }

      await fs.rm(tempPath, { force: true })

      bin = path.join(interbaseRuntimeContext.paths.bin, "tinymist" + (platform === "win32" ? ".exe" : ""))
