...
Replace runtime Bun shell template-tag usage in `packages/cli-host/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/cli-host/src` is removed except:
