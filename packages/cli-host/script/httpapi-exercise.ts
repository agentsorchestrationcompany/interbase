 ...
 *
 * The script intentionally isolates `INTERBASE_DB` before importing modules that touch
...
const preserveExerciseGlobalRoot = !!process.env.INTERBASE_HTTPAPI_EXERCISE_GLOBAL
const exerciseGlobalRoot =
  process.env.INTERBASE_HTTPAPI_EXERCISE_GLOBAL ??
  path.join(process.env.TMPDIR ?? "/tmp", `interbase-httpapi-global-${process.pid}`)
process.env.XDG_DATA_HOME = path.join(exerciseGlobalRoot, "data")
process.env.XDG_CONFIG_HOME = path.join(exerciseGlobalRoot, "config")
process.env.XDG_STATE_HOME = path.join(exerciseGlobalRoot, "state")
process.env.XDG_CACHE_HOME = path.join(exerciseGlobalRoot, "cache")
process.env.INTERBASE_DISABLE_SHARE = "true"
const exerciseConfigDirectory = path.join(exerciseGlobalRoot, "config", "interbase")
const exerciseDataDirectory = path.join(exerciseGlobalRoot, "data", "interbase")

const preserveExerciseDatabase = !!process.env.INTERBASE_HTTPAPI_EXERCISE_DB
const exerciseDatabasePath =
  process.env.INTERBASE_HTTPAPI_EXERCISE_DB ??
  path.join(process.env.TMPDIR ?? "/tmp", `interbase-httpapi-exercise-${process.pid}.db`)
process.env.INTERBASE_DB = exerciseDatabasePath
Flag.INTERBASE_DB = exerciseDatabasePath

void (await import("@interbase/core/util/log")).init({ print: false })
...
const original = {
  INTERBASE_EXPERIMENTAL_HTTPAPI: Flag.INTERBASE_EXPERIMENTAL_HTTPAPI,
  INTERBASE_SERVER_PASSWORD: Flag.INTERBASE_SERVER_PASSWORD,
  INTERBASE_SERVER_USERNAME: Flag.INTERBASE_SERVER_USERNAME,
}
...
const scenarios: Scenario[] = [
  ...
  http
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "interbase.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "interbase.jsonc")).text(),
          )
  ...
  http.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-interbase-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-interbase-directory")
  }),
  ...
  http
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "interbase", model: "test" })}`,
      headers: ctx.headers(),
    }))
  ...
  http
    ...
    .at((ctx) => ({
      ...
      headers: ctx.headers(),
      body: { command: "review", arguments: "", model: "test/test-model" },
    }))
    .jsonEffect(
      200,
      (body, ctx) =>
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-interbase-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
          ...
          message: (sessionID, input) =>
            Effect.gen(function* () {
              const info: MessageV2.User = {
                ...
                agent: "build",
                model: {
                  providerID: ProviderID.interbase,
...
function app(modules: Runtime, backend: Backend) {
  Flag.INTERBASE_EXPERIMENTAL_HTTPAPI = backend === "effect"
  Flag.INTERBASE_SERVER_PASSWORD = undefined
  Flag.INTERBASE_SERVER_USERNAME = undefined
    ...
    modules.ExperimentalHttpApiServer.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ INTERBASE_SERVER_PASSWORD: undefined, INTERBASE_SERVER_USERNAME: undefined }),
        ),
      ),
    ),
    { disableLogger: true },
...
const resetState = Effect.promise(async () => {
  ...
  Flag.INTERBASE_EXPERIMENTAL_HTTPAPI = original.INTERBASE_EXPERIMENTAL_HTTPAPI
  Flag.INTERBASE_SERVER_PASSWORD = original.INTERBASE_SERVER_PASSWORD
  Flag.INTERBASE_SERVER_USERNAME = original.INTERBASE_SERVER_USERNAME
