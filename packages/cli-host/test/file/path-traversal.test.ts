...
describe("Filesystem.contains", () => {
  ...
  test("blocks absolute paths outside project", () => {
    ...
    expect(Filesystem.contains("/tmp/project", "/tmp/other")).toBe(false)
  })
...
describe("containsPath", () => {
  ...
  test("returns true for path inside worktree but outside directory (monorepo subdirectory scenario)", async () => {
    ...
    await WithInstance.provide({
      directory: subdir,
      fn: () => {
        // .interbase at worktree root, but we're running from packages/lib
        expect(containsPath(path.join(tmp.path, ".interbase", "state"), Instance.current)).toBe(true)
