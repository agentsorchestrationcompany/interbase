...
import { INTERBASE_KEYBIND_DEFAULTS } from "@interbase/overlay"

describe("ConfigKeybinds", () => {
  test("uses overlay-owned Interbase prompt defaults", () => {
    expect(ConfigKeybinds.Keybinds.shape.agent_cycle.parse(undefined)).toBe(INTERBASE_KEYBIND_DEFAULTS.agent_cycle)
    expect(ConfigKeybinds.Keybinds.shape.agent_cycle_reverse.parse(undefined)).toBe(
      INTERBASE_KEYBIND_DEFAULTS.agent_cycle_reverse,
    )
    expect(ConfigKeybinds.Keybinds.shape.input_submit.parse(undefined)).toBe(INTERBASE_KEYBIND_DEFAULTS.input_submit)
  })
})
