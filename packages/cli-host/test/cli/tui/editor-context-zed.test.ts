...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.INTERBASE_ZED_DB
  process.env.INTERBASE_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.INTERBASE_ZED_DB
    else process.env.INTERBASE_ZED_DB = previous
    home.mockRestore()
  }
})
