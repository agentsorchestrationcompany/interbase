...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".interbase", "themes", localThemeFile)
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".interbase", "themes", preloadedThemeFile)
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".interbase", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
    ...
    return {
      ...
      provider_dialog_opened: providerDialogOpened,
    }
  } finally {
...
`,
  ...
  process.env.INTERBASE_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config: TuiConfig.Info = {
    plugin: [
  ...
  try {
    ...
    cwd.mockRestore()
    wait.mockRestore()
    delete process.env.INTERBASE_PLUGIN_META_FILE
  }
})
...
describe("tui.plugin.loader", () => {
  ...
  test("passes keybind, kv, state, and dialog APIs to v1 plugins", () => {
    ...
    expect(data.provider_dialog_opened).toBe(1)
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".interbase", "themes", themeFile)
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.INTERBASE_PLUGIN_META_FILE!,
    )
