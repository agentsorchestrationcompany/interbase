...
test("GitHub action does not call the hosted Interbase GitHub App backend", () => {
  const source = readFileSync(new URL("../../src/cli/cmd/github.ts", import.meta.url), "utf8")

  expect(source).not.toContain("api.interbase.ai")
  expect(source).not.toContain("get_github_app_installation")
  expect(source).not.toContain("exchange_github_app_token")
  expect(source).not.toContain("interbase-github-action")
