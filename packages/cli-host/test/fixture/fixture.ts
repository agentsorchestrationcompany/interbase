...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const root = process.env.INTERBASE_TEST_SANDBOX_ROOT?.trim() || os.tmpdir()
  const dirpath = sanitizePath(path.join(root, "interbase-test-" + Math.random().toString(36).slice(2)))
  await fs.mkdir(dirpath, { recursive: true })
  if (options?.git) {
    await $`git init`.cwd(dirpath).quiet()
    await $`git config core.fsmonitor false`.cwd(dirpath).quiet()
    await $`git config commit.gpgsign false`.cwd(dirpath).quiet()
    await $`git config user.email "test@interbase.test"`.cwd(dirpath).quiet()
    await $`git config user.name "Test"`.cwd(dirpath).quiet()
    await $`git commit --allow-empty -m "root commit ${dirpath}"`.cwd(dirpath).quiet()
  }
  if (options?.config) {
    await Bun.write(
      path.join(dirpath, "interbase.json"),
      JSON.stringify({
...
export function tmpdirScoped(options?: { git?: boolean; config?: Partial<Config.Info> }) {
  return Effect.gen(function* () {
    ...
    const dirpath = sanitizePath(path.join(os.tmpdir(), "interbase-test-" + Math.random().toString(36).slice(2)))
    ...
    if (options?.git) {
      ...
      yield* git("config", "user.email", "test@interbase.test")
    ...
    if (options?.config) {
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://interbase.ai/config.json", ...options.config }),
        ),
      )
    }
