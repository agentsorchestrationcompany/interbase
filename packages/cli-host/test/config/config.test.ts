...
const managedConfigDir = process.env.INTERBASE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "interbase.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "interbase.json") {
...
async function check(map: (dir: string) => string) {
  ...
  const prev = process.env.INTERBASE_CONFIG_DIR
  process.env.INTERBASE_CONFIG_DIR = globalTmp.path
  await clear()
  try {
    ...
    if (prev === undefined) delete process.env.INTERBASE_CONFIG_DIR
    else process.env.INTERBASE_CONFIG_DIR = prev
    await clear()
  }
}
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  const prev = process.env.INTERBASE_CONFIG_DIR
  process.env.INTERBASE_CONFIG_DIR = tmp.path
  await clear(true)

  try {
    await saveGlobal({ shell: "" })

    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "interbase.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "interbase.jsonc")
...
test("ignores legacy tui keys in interbase config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://interbase.ai/config.json",
  ...
  try {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "interbase.json"))
...
test("ignores invalid discovered project config fields", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://interbase.ai/config.json",
...
test("ignores invalid JSON in discovered project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "interbase.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("throws error for invalid local .interbase config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".interbase"), { recursive: true })
      await Filesystem.write(path.join(dir, ".interbase", "interbase.json"), "{ invalid json }")
    },
  })
  await provideTestInstance({
    directory: tmp.path,
...
test("ignores removed share settings", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "interbase.json"),
  ...
  await WithInstance.provide({
    ...
    fn: async () => {
      ...
      expect((config as any).share).toBeUndefined()
      expect((config as any).autoshare).toBeUndefined()
    },
  })
})
...
test("loads config from .interbase directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const interbaseDir = path.join(dir, ".interbase")
      await fs.mkdir(interbaseDir, { recursive: true })
      const agentDir = path.join(interbaseDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".interbase", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .interbase/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const interbaseDir = path.join(dir, ".interbase")
      await fs.mkdir(interbaseDir, { recursive: true })

      const agentsDir = path.join(interbaseDir, "agents")
...
test("loads commands from .interbase/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const interbaseDir = path.join(dir, ".interbase")
      await fs.mkdir(interbaseDir, { recursive: true })

      const commandDir = path.join(interbaseDir, "command")
...
test("loads commands from .interbase/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const interbaseDir = path.join(dir, ".interbase")
      await fs.mkdir(interbaseDir, { recursive: true })

      const commandsDir = path.join(interbaseDir, "commands")
...
test("does not try to install dependencies in read-only OPENCODE_CONFIG_DIR", async () => {
  ...
  const prev = process.env.INTERBASE_CONFIG_DIR
  process.env.INTERBASE_CONFIG_DIR = tmp.extra
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "interbase.json"),
        JSON.stringify({ $schema: "https://interbase.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .interbase config
      const projectDir = path.join(dir, "project")
      const interbaseDir = path.join(projectDir, ".interbase")
      await fs.mkdir(interbaseDir, { recursive: true })
      ...
      // Local .interbase config with different plugins
      await Filesystem.write(
        path.join(interbaseDir, "interbase.json"),
        JSON.stringify({
          $schema: "https://interbase.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .interbase config with some overlapping plugins
      await Filesystem.write(
        path.join(interbaseDir, "interbase.json"),
        JSON.stringify({
          $schema: "https://interbase.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".interbase")
      ...
      await Filesystem.write(
        path.join(local, "interbase.json"),
        JSON.stringify({
          $schema: "https://interbase.ai/config.json",
...
test("local .interbase config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .interbase directory config enables it
      const interbaseDir = path.join(dir, ".interbase")
      await fs.mkdir(interbaseDir, { recursive: true })
      await Filesystem.write(
        path.join(interbaseDir, "interbase.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/interbase")) {
      fetchedUrl = urlStr
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe("https://example.com/.well-known/interbase")
...
test("remote well-known config is cached across instance reloads", async () => {
  const originalFetch = globalThis.fetch
  let fetchCount = 0
  const mockedFetch: typeof fetch = Object.assign(
    mock((url: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
      const urlStr = url instanceof Request ? url.url : url instanceof URL ? url.href : url
      if (urlStr.includes(".well-known/interbase")) {
        fetchCount++
        return Promise.resolve(
          new Response(
            JSON.stringify({
              config: {
                mcp: { docs: { type: "remote", url: "https://docs.example.com/mcp", enabled: true } },
              },
            }),
            { status: 200 },
          ),
...
test("unavailable remote well-known config is ignored", async () => {
  const originalFetch = globalThis.fetch
  let fetchCount = 0
  const mockedFetch: typeof fetch = Object.assign(
    mock((url: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(config.mcp?.local?.enabled).toBe(true)
            expect(fetchCount).toBe(1)
          }),
        ),
      { git: true, config: { mcp: { local: { type: "local", command: ["node", "server.js"], enabled: true } } } },
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.runPromise)
  } finally {
    globalThis.fetch = originalFetch
  }
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "interbase.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-interbase@2.4.3", file)).toBe("oh-my-interbase@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-interbase@2.4.3", "file:///project/.interbase/plugin/oh-my-interbase.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.interbase/plugin/demo.ts", "file:///project/.interbase/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.interbase/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const interbaseDir = path.join(projectDir, ".interbase")
        const pluginDir = path.join(interbaseDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["INTERBASE_DISABLE_PROJECT_CONFIG"]
    process.env["INTERBASE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["INTERBASE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["INTERBASE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .interbase/ directories when flag is set", async () => {
    const originalEnv = process.env["INTERBASE_DISABLE_PROJECT_CONFIG"]
    process.env["INTERBASE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .interbase directory with a command
          const interbaseDir = path.join(dir, ".interbase", "command")
          await fs.mkdir(interbaseDir, { recursive: true })
          await Filesystem.write(path.join(interbaseDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await WithInstance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .interbase should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["INTERBASE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["INTERBASE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["INTERBASE_CONFIG_DIR"]
      process.env["INTERBASE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["INTERBASE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["INTERBASE_CONFIG_DIR"]
      } else {
        process.env["INTERBASE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })
  ...
  test("OPENCODE_CONFIG_DIR still works when flag is set", async () => {
    ...
    try {
      ...
      process.env["INTERBASE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["INTERBASE_CONFIG_DIR"] = configDirTmp.path
...
describe("INTERBASE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in INTERBASE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["INTERBASE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["INTERBASE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://interbase.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["INTERBASE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["INTERBASE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in INTERBASE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["INTERBASE_CONFIG_CONTENT"]
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          ...
          PayloadIdentifier: "ai.interbase.managed.test",
          PayloadType: "ai.interbase.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://interbase.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://interbase.ai/config.json")
})
