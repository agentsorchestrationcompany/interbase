...
const disableDefault = process.env.INTERBASE_DISABLE_DEFAULT_PLUGINS
process.env.INTERBASE_DISABLE_DEFAULT_PLUGINS = "1"

const { Plugin } = await import("../../src/plugin/index")
const { PluginLoader } = await import("../../src/plugin/loader")
const { checkPluginCompatibility, readPackageThemes } = await import("../../src/plugin/shared")
const { Bus } = await import("../../src/bus")
const { Npm } = await import("@interbase/core/npm")
const { TestConfig } = await import("../fixture/config")

afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.INTERBASE_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.INTERBASE_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
async function load(dir: string) {
  const source = path.join(dir, "interbase.json")
...
describe("plugin.loader.shared", () => {
  ...
  test("rejects incompatible npm plugin engines before module import", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const mod = path.join(dir, "mods", "acme-plugin")
        const mark = path.join(dir, "imported.txt")
        ...
        await Bun.write(path.join(mod, "index.js"), `await Bun.write(${JSON.stringify(mark)}, "imported")\n`)
        return { mark, mod }
      },
    })

    await expect(checkPluginCompatibility(tmp.extra.mod, "1.2.3")).rejects.toThrow(
      "Plugin requires interbase >=99.0.0 but running 1.2.3",
    )
    const imported = await Bun.file(tmp.extra.mark)
      .text()
      .then(() => true)
      .catch(() => false)
  ...
  test("loads npm server plugin from package ./server export", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "interbase.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  test("rejects npm server export that resolves outside plugin directory", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "interbase.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
  ...
  test("skips legacy codex and copilot auth plugin specs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["interbase-openai-codex-auth@1.0.0", "interbase-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
    ...
    try {
      ...
      expect(pkgs).not.toContain("interbase-openai-codex-auth@1.0.0")
      expect(pkgs).not.toContain("interbase-copilot-auth@1.0.0")
    } finally {
      install.mockRestore()
    }
  })
  ...
  test("continues loading plugins when plugin init throws", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "interbase.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  test("continues loading plugins when plugin import fails", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "interbase.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "interbase.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
  ...
  test("skips external plugins in pure mode", async () => {
    ...
    const pure = process.env.INTERBASE_PURE
    ...
    try {
      ...
      if (pure === undefined) {
        delete process.env.INTERBASE_PURE
      } else {
        process.env.INTERBASE_PURE = pure
      }
    }
  })
