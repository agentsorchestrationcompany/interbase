...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "interbase.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "interbase.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "interbase.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "interbase.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["interbase.json", "interbase.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "interbase.json"),
        path.join(tmp.path, "interbase.jsonc"),
        path.join(project, "interbase.json"),
        path.join(project, "interbase.jsonc"),
      ])
    })
  })
  ...
  describe("windowsPath()", () => {
    test("converts Git Bash paths", () => {
      if (process.platform === "win32") {
        expect(Filesystem.windowsPath("/c/workspace/test")).toBe("C:/workspace/test")
    ...
    test("converts Cygwin paths", () => {
      if (process.platform === "win32") {
        expect(Filesystem.windowsPath("/cygdrive/c/workspace/test")).toBe("C:/workspace/test")
        expect(Filesystem.windowsPath("/cygdrive/x/dev/project")).toBe("X:/dev/project")
      } else {
        expect(Filesystem.windowsPath("/cygdrive/c/workspace/test")).toBe("/cygdrive/c/workspace/test")
      }
    })

    test("converts WSL paths", () => {
      if (process.platform === "win32") {
        expect(Filesystem.windowsPath("/mnt/c/workspace/test")).toBe("C:/workspace/test")
        expect(Filesystem.windowsPath("/mnt/z/dev/project")).toBe("Z:/dev/project")
      } else {
        expect(Filesystem.windowsPath("/mnt/c/workspace/test")).toBe("/mnt/c/workspace/test")
      }
    })
