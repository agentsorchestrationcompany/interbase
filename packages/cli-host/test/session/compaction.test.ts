...
describe("session.compaction.isOverflow", () => {
  ...
  it.live(
    "uses a Codex-like default auto-compact threshold at 90% of resolved context",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 100_000, output: 1 })
        expect(
  ...
  it.live(
    "allows configured auto-compact threshold override",
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const compact = yield* SessionCompaction.Service
          ...
          expect(
            yield* compact.isOverflow({
              tokens: { input: 49_999, output: 0, reasoning: 0, cache: { read: 0, write: 0 } },
              model,
            }),
          ).toBe(false)
          expect(
  ...
  it.live(
    "Total scope counts full active context",
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const compact = yield* SessionCompaction.Service
          const model = createModel({ context: 100_000, output: 1 })
          const tokens = { input: 10_000, output: 0, reasoning: 0, cache: { read: 81_000, write: 0 } }
          expect(yield* compact.isOverflow({ tokens, model })).toBe(true)
        }),
      {
        config: {
          compaction: { scope: "total" },
        },
      },
    ),
  )
  ...
  it.live(
    "BodyAfterPrefix scope counts only growth after the cached prefix for the soft threshold",
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const compact = yield* SessionCompaction.Service
      ...
      {
        config: {
          compaction: { scope: "body_after_prefix" },
        },
      },
    ),
  )
  ...
  it.live(
    "BodyAfterPrefix scope keeps a hard full-context safety guard",
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const compact = yield* SessionCompaction.Service
          const model = createModel({ context: 100_000, output: 1 })
          const tokens = { input: 10_000, output: 0, reasoning: 0, cache: { read: 90_000, write: 0 } }
          expect(yield* compact.isOverflow({ tokens, model })).toBe(true)
        }),
      {
        config: {
  ...
  it.live(
    "without limit.input, same token count also triggers compaction",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 200_000, output: 32_000 })
  ...
  it.live(
    "limit.input and equivalent context-only models compact symmetrically",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
...
describe("session.compaction.create", () => {
  ...
  it.live(
    "records manual user-requested phase and reason",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const ssn = yield* SessionNs.Service
        ...
        const msg = (yield* ssn.messages({ sessionID: info.id })).at(-1)
...
describe("session.compaction.process", () => {
  ...
  test("serializes compaction history as transcript text instead of tool protocol", async () => {
    const stub = llm()
    let captured = ""
    stub.push(
      reply("summary", (input) => {
    ...
    await WithInstance.provide({
      ...
      fn: async () => {
        const session = await svc.create({})
        const first = await user(session.id, "Investigate the mobile chat delay.")
        const answer = await assistant(session.id, first.id, tmp.path)
        await svc.updatePart({
          id: PartID.ascending(),
          messageID: answer.id,
          sessionID: session.id,
          type: "text",
          text: "I traced the first page fetch path.",
        })
        await svc.updatePart({
          id: PartID.ascending(),
          messageID: answer.id,
          ...
          state: {
            status: "completed",
            input: { filePath: "packages/cli-host/src/provider/provider.ts" },
            output: "Line 1064: re_pair\nLine 1065: retry",
            title: "",
            metadata: {},
            time: { start: Date.now(), end: Date.now() },
          },
        ...
        await svc.updatePart({
          ...
          type: "patch",
          hash: "abc123",
          files: ["packages/cli-host/src/index.ts"],
        })
        await user(session.id, "Keep the most recent turn out of the summary.")
        await SessionCompaction.create({
          sessionID: session.id,
          agent: "build",
          model: ref,
        ...
        try {
          ...
          expect(captured).toContain("Investigate the mobile chat delay.")
          expect(captured).toContain("I traced the first page fetch path.")
          expect(captured).toContain("Tool: read")
          expect(captured).toContain("Applied patch: packages/cli-host/src/index.ts")
          expect(captured).not.toContain('"filePath":"packages/cli-host/src/provider/provider.ts"')
          expect(captured).not.toContain("Line 1064: re_pair")
          expect(captured).not.toContain("private chain of thought")
          expect(captured).not.toContain("function_call_output")
          expect(captured).not.toContain("What did we do so far?")
        } finally {
          await rt.dispose()
        }
