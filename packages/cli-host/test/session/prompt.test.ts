...
import { MainDbGoalStore } from "../../src/session/goal-store"
...
function writeGoal(
  sessionID: SessionID,
  input?: {
    status?: "active" | "paused" | "blocked" | "usageLimited" | "budgetLimited"
    objective?: string
    tokenBudget?: number | null
    tokensUsed?: number
  },
) {
  new MainDbGoalStore().set({
    threadId: sessionID,
    goalId: `goal-${sessionID}`,
    objective: input?.objective ?? "Continue parity work",
...
it.live("goal continuation waits for pending user questions", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const question = yield* Question.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Goal question" })
      yield* seed(chat.id, { finish: "stop" })

      writeGoal(chat.id, { objective: "Wait for confirmation before continuing" })

      const pendingQuestion = yield* question
        .ask({
          sessionID: chat.id,
          questions: [
            {
              header: "Confirm",
              question: "Continue?",
              options: [{ label: "Continue", description: "Resume the goal" }],
            },
          ],
        })
        .pipe(Effect.forkScoped)
      let pending = yield* question.list()
      for (let i = 0; i < 20 && pending.length === 0; i++) {
        yield* Effect.sleep(10)
        pending = yield* question.list()
      }
      expect(pending).toHaveLength(1)

      yield* llm.text("should not run while a question is pending")
      const result = yield* prompt.loop({ sessionID: chat.id })
      expect(result.info.role).toBe("assistant")
      expect(yield* llm.calls).toBe(0)

      yield* question.reject(pending[0]!.id)
      yield* Fiber.await(pendingQuestion)
    }),
    { git: true, config: providerCfg },
  ),
)
...
it.live(
  "active goal idle continuation near threshold compacts before a continuation model call",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const chat = yield* sessions.create({ title: "Goal compaction" })
        yield* seed(chat.id, {
          finish: "stop",
        ...
        expect(JSON.stringify(inputs[0]?.messages)).toContain("Output exactly the Markdown structure")
        expect(JSON.stringify(inputs[0]?.messages)).not.toContain("<goal_context>")
        ...
        expect(
          msgs
            .flatMap((msg) => msg.parts)
            .some((part) => part.type === "text" && part.synthetic === true && part.text.includes("<goal_context>")),
        ).toBe(false)
...
it.live("prompt without explicit model uses persisted session model", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Persisted model" })

      yield* sessions.setModel({
        sessionID: chat.id,
        model: {
          id: ModelID.make("switched-model"),
          providerID: ProviderID.make("test"),
          variant: "fast",
        },
      })
      ...
      const users = messages.filter((message) => message.info.role === "user").map((message) => message.info)
      expect(users).toHaveLength(2)
      expect(
        users.map((user) =>
          user.role === "user"
            ? { providerID: user.model.providerID, modelID: user.model.modelID, variant: user.model.variant }
            : undefined,
        ),
      ).toEqual([
        { providerID: ProviderID.make("test"), modelID: ModelID.make("switched-model"), variant: "fast" },
        { providerID: ProviderID.make("test"), modelID: ModelID.make("switched-model"), variant: "fast" },
      ])
    }),
    { git: true, config: providerCfg },
...
it.live("model downshift near the new context limit compacts with the previous model before sampling", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const chat = yield* sessions.create({ title: "Model downshift" })
      yield* seed(chat.id, {
        finish: "stop",
        tokens: { input: 45_000, output: 0, reasoning: 0, cache: { read: 0, write: 0 } },
      })
      yield* sessions.setModel({
        sessionID: chat.id,
        model: {
      ...
      const fiber = yield* prompt
        .prompt({
          sessionID: chat.id,
          agent: "build",
          parts: [{ type: "text", text: "continue on the smaller model" }],
        })
      ...
      const inputs = yield* llm.inputs
      expect(JSON.stringify(inputs[0]?.messages)).toContain("Output exactly the Markdown structure")
      expect(inputs[1]?.model).toBe("switched-model")

      const msgs = yield* sessions.messages({ sessionID: chat.id })
      const compactionMsg = msgs.find((msg) => msg.parts.some((part) => part.type === "compaction"))
      expect(compactionMsg?.info.role).toBe("user")
      if (compactionMsg?.info.role === "user") {
        expect(compactionMsg.info.model).toMatchObject({
          providerID: ProviderID.make("test"),
          modelID: ModelID.make("test-model"),
        })
      }
      expect(
        compactionMsg?.parts.some(
          (part) =>
            part.type === "compaction" &&
            part.auto === true &&
            part.phase === "preTurn" &&
            part.reason === "modelDownshift" &&
            part.resume_model?.modelID === ModelID.make("switched-model"),
        ),
      ).toBe(true)
      expect(
        msgs.some(
          (msg) =>
            msg.info.role === "user" &&
            msg.info.model.modelID === ModelID.make("switched-model") &&
            msg.parts.some((part) => part.type === "text" && part.text === "continue on the smaller model"),
        ),
      ).toBe(true)
    ...
    }),
    {
      ...
      config: (url) => {
        ...
        next.provider.test.models["switched-model"] = {
          ...next.provider.test.models["switched-model"],
          limit: { context: 50_000, output: 10_000 },
        }
        return next
      },
    },
...
it.live("budget-limited active goal steers before tool-call follow-up", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      const session = yield* sessions.create({
        title: "Goal budget steering",
        permission: [{ permission: "*", pattern: "*", action: "allow" }],
      })
      writeGoal(session.id, { tokenBudget: 5 })
      yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        noReply: true,
        parts: [{ type: "text", text: "find files for goal" }],
      })
      yield* llm.push(reply().tool("glob", { pattern: "**/*.txt" }).usage({ input: 10, output: 0 }))
      yield* llm.text("wrapped up")

      const result = yield* prompt.loop({ sessionID: session.id })

      expect(yield* llm.calls).toBe(2)
      expect(result.parts.some((part) => part.type === "text" && part.text === "wrapped up")).toBe(true)
      const inputs = yield* llm.inputs
      expect(JSON.stringify(inputs[1]?.messages)).toContain("has reached its token budget")
      expect(JSON.stringify(inputs[1]?.messages)).toContain("budget_limited")
      ...
      expect(
        msgs
          ...
          .some(
            (part) =>
              part.type === "text" && part.synthetic === true && part.text.includes("has reached its token budget"),
          ),
      ).toBe(true)
    }),
    { git: true, config: providerCfg },
...
it.live("tool-call follow-up near threshold compacts before the follow-up model call", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      const prompt = yield* SessionPrompt.Service
      const sessions = yield* Session.Service
      ...
      yield* llm.push(reply().tool("glob", { pattern: "**/*.txt" }).usage({ input: 90_000, output: 0 }))
      yield* llm.hang
      ...
      const inputs = yield* llm.inputs
      expect(JSON.stringify(inputs[0]?.messages)).toContain("list text files")
      expect(JSON.stringify(inputs[1]?.messages)).toContain("Output exactly the Markdown structure")
      ...
      Effect.fnUntraced(function* ({ llm }) {
        ...
        const a = yield* prompt
          .prompt({
            ...
            agent: "build",
            model: ref,
            parts: [{ type: "text", text: "explain the latest commit" }],
          })
          .pipe(Effect.forkChild)
        ...
        const b = yield* prompt
          .prompt({
            ...
            agent: "build",
            model: ref,
            parts: [{ type: "text", text: "how many commits took place today?" }],
          })
          .pipe(Effect.forkChild)
        ...
        expect(JSON.stringify(beforeRelease.at(0)?.messages)).toContain("explain the latest commit")
        expect(JSON.stringify(beforeRelease.at(0)?.messages)).not.toContain("how many commits took place today?")
        ...
        yield* Effect.promise(() => new Promise((done) => setTimeout(done, 250)))
        ...
        expect(JSON.stringify(inputs.at(0)?.messages)).toContain("explain the latest commit")
        expect(JSON.stringify(inputs.at(1)?.messages)).toContain("how many commits took place today?")
      }),
      { git: true, config: providerCfg },
    ),
  10_000,
...
it.live(
  "queued prompt waits for a pending question before draining the next turn",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        ...
        const sessions = yield* Session.Service
        const status = yield* SessionStatus.Service
        const chat = yield* sessions.create({ title: "Question queue" })
          ...
          reply()
            .tool("question", {
              questions: [
                {
                  header: "Confirm",
                  question: "Continue with the first task?",
                  custom: false,
                  options: [{ label: "Continue", description: "Finish the active task first" }],
                },
              ],
            })
        ...
        yield* llm.text("first answer after question")
        ...
        const active = yield* prompt
          .prompt({
            ...
            agent: "build",
            model: ref,
            parts: [{ type: "text", text: "handle the first request" }],
          })
          .pipe(Effect.forkChild)
        ...
        const queued = yield* prompt
          .prompt({
            ...
            agent: "build",
            model: ref,
            parts: [{ type: "text", text: "handle the queued request" }],
          })
          .pipe(Effect.forkChild)
        ...
        expect(yield* llm.calls).toBe(1)
        expect(yield* status.get(chat.id)).toEqual({ type: "busy", queued: [queuedID] })

        const beforeReply = yield* llm.inputs
        expect(beforeReply).toHaveLength(1)
        expect(JSON.stringify(beforeReply[0]?.messages)).toContain("handle the first request")
        expect(JSON.stringify(beforeReply[0]?.messages)).not.toContain("handle the queued request")

        yield* question.reply({ requestID: pending[0]!.id, answers: [["Continue"]] })

        const activeExit = yield* Fiber.await(active)
        expect(Exit.isSuccess(activeExit)).toBe(true)
        if (Exit.isSuccess(activeExit)) {
          expect(activeExit.value.info.role).toBe("assistant")
          expect(
            activeExit.value.parts.some((part) => part.type === "text" && part.text === "first answer after question"),
          ).toBe(true)
        }
        ...
        const last = assistants.at(-1)
        if (!last || last.info.role !== "assistant") throw new Error("expected queued assistant")
        expect(last.info.parentID).toBe(queuedID)
        expect(last.parts.some((part) => part.type === "text" && part.text === "second queued answer")).toBe(true)

        const afterReply = yield* llm.inputs
        expect(afterReply).toHaveLength(3)
        expect(JSON.stringify(afterReply[1]?.messages)).not.toContain("handle the queued request")
        expect(JSON.stringify(afterReply[2]?.messages)).toContain("handle the queued request")
      }),
      { git: true, config: providerCfg },
    ),
  10_000,
...
it.live(
  "active goal idle continuation waits for queued user input",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const gate = defer<void>()
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const chat = yield* sessions.create({ title: "Goal queue" })
        writeGoal(chat.id)
        ...
        const active = yield* prompt
          .prompt({
            ...
            agent: "build",
            model: ref,
            parts: [{ type: "text", text: "first goal turn" }],
          })
          .pipe(Effect.forkChild)
        yield* llm.wait(1)
        ...
        yield* prompt
          .prompt({
            ...
            agent: "build",
            model: ref,
            parts: [{ type: "text", text: "queued user turn wins" }],
          })
          .pipe(Effect.forkChild)
        ...
        const inputs = yield* llm.inputs
        expect(JSON.stringify(inputs[1]?.messages)).toContain("queued user turn wins")
        expect(JSON.stringify(inputs[1]?.messages)).not.toContain("<goal_context>")
...
it.live(
  "goal resume command without a goal renders a command result without running the model",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        ...
        expect(
          result.parts.some(
            (part) =>
              part.type === "text" &&
              part.text === "No active goal exists to resume." &&
              part.synthetic === true &&
              part.metadata?.kind === "interbase_command_result" &&
              part.metadata.title === "No active goal exists to resume.",
          ),
        ).toBe(true)
        expect(yield* llm.calls).toBe(0)
      }),
...
it.live(
  "goal command schedules continuation as a hidden normal turn",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const chat = yield* sessions.create({ title: "Goal scheduled continuation" })
        yield* llm.text("continued")

        const result = yield* prompt.command({
          sessionID: chat.id,
          command: "goal",
          arguments: "ship scheduled continuation parity",
          agent: "build",
        })
        yield* llm.wait(1)
        ...
        expect(
          result.parts.some(
            (part) =>
              part.type === "text" &&
              part.text === "Goal Active\nObjective: ship scheduled continuation parity" &&
              part.synthetic === true &&
              part.metadata?.kind === "interbase_command_result",
          ),
        ).toBe(true)
        const inputs = yield* llm.inputs
        expect(JSON.stringify(inputs[0]?.messages)).toContain("<goal_context>")
        expect(JSON.stringify(inputs[0]?.messages)).toContain("ship scheduled continuation parity")
        const msgs = yield* sessions.messages({ sessionID: chat.id })
        expect(
          msgs
            .flatMap((msg) => msg.parts)
...
it.live(
  "goal resume command near threshold compacts before the continuation model call",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const chat = yield* sessions.create({ title: "Resume compaction" })
        yield* seed(chat.id, {
          finish: "stop",
          tokens: { input: 89_999, output: 0, reasoning: 0, cache: { read: 0, write: 0 } },
        })
        writeGoal(chat.id, { status: "paused" })
        yield* llm.hang
...
it.live(
  "goal resume command waits for pending questions before scheduling continuation",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        const prompt = yield* SessionPrompt.Service
        const question = yield* Question.Service
        const sessions = yield* Session.Service
        const chat = yield* sessions.create({ title: "Resume pending question" })
        yield* seed(chat.id, { finish: "stop" })
        writeGoal(chat.id, { status: "paused" })
        yield* question
          .ask({
            ...
            questions: [
              {
                header: "Wait",
                question: "Hold continuation?",
                custom: false,
                options: [
                  { label: "Yes", description: "Keep waiting" },
                  { label: "No", description: "Continue now" },
                ],
              },
            ],
          })
...
it.live(
  "command plugin confirmation asks question and retries confirmed command",
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* () {
        const prompt = yield* SessionPrompt.Service
        ...
        expect(
          first.parts.some(
            (part) =>
              part.type === "text" &&
              part.text.includes("Goal Active") &&
              part.metadata?.kind === "interbase_command_result" &&
              part.metadata.title === "Goal Active" &&
              part.metadata.detail === "Objective: ship goal parity",
          ),
        ).toBe(true)
        ...
        const replacement = yield* prompt
          .command({
            sessionID: chat.id,
            command: "goal",
            arguments: "ship confirmed goal parity",
            agent: "build",
          })
          .pipe(Effect.forkScoped)
        ...
        expect(pending).toHaveLength(1)
        expect(pending[0]?.questions[0]?.question).toBe("Replace goal?")
        expect(pending[0]?.questions[0]?.options).toEqual([
          { label: "Replace current goal", description: "Set the new objective and start it now" },
          { label: "Cancel", description: "Keep the current goal" },
        ])
        yield* question.reply({ requestID: pending[0]!.id, answers: [["Replace current goal"]] })

        const exit = yield* Fiber.await(replacement)
        expect(Exit.isSuccess(exit)).toBe(true)
        if (Exit.isSuccess(exit)) {
          expect(
            exit.value.parts.some((part) => part.type === "text" && part.text.includes("ship confirmed goal parity")),
          ).toBe(true)
        }
      }),
      { git: true, config: providerCfg },
...
it.live("does not loop empty assistant turns for a simple reply", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* ({ llm }) {
      ...
      yield* llm.text("packages/cli-host/src/session/processor.ts")
  ...
  provideTmpdirInstance(
    (_dir) =>
      Effect.gen(function* () {
        const prompt = yield* SessionPrompt.Service
        const sessions = yield* Session.Service
        const explicitSession = yield* sessions.create({})

        const other = yield* prompt.prompt({
          sessionID: explicitSession.id,
          agent: "build",
          model: { providerID: ProviderID.make("interbase"), modelID: ModelID.make("kimi-k2.5-free") },
          noReply: true,
          parts: [{ type: "text", text: "hello" }],
        })
        ...
        yield* sessions.remove(explicitSession.id)
        yield* sessions.remove(session.id)
      }),
    {
      git: true,
