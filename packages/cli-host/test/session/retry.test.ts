...
import type { NamedError } from "@interbase/core/util/error"
...
describe("session.message-v2.fromError", () => {
  ...
  test("marks insufficient_quota API errors as non-retryable", () => {
    const error = new APICallError({
    ...
    expect(result.data.usageLimited).toBe(true)
    expect(SessionRetry.retryable(result)).toBeUndefined()
  })

  test("marks type-only quota API errors as non-retryable", () => {
    const error = new APICallError({
      message: "429 You exceeded your current quota",
      url: "https://api.openai.com/v1/chat/completions",
      requestBodyValues: {},
  ...
  test("marks quota-exhaustion message errors as non-retryable", () => {
    const error = new APICallError({
  ...
  test("retries transient rate-limit 429s with no quota keywords", () => {
    const error = new APICallError({
      ...
      responseBody: JSON.stringify({
        error: {
          type: "rate_limit_error",
          message: "Your request was rate limited. Please wait before retrying.",
        },
      }),
      isRetryable: true,
    })
