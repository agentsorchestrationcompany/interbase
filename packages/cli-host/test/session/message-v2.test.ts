...
function assistantInfo(
  ...
  meta?: { apiID?: string; apiPackage?: string; providerID: string; modelID: string },
): MessageV2.Assistant {
  ...
  return {
    ...
    error,
    parentID,
    apiID: infoModel.apiID,
    apiPackage: infoModel.apiPackage,
...
describe("session.message-v2.toModelMessage", () => {
  ...
  test("omits provider metadata when assistant API differs despite same provider and model", async () => {
    const userID = "m-user"
    const assistantID = "m-assistant"
    ...
    const input: MessageV2.WithParts[] = [
      {
        info: assistantInfo(assistantID, "m-parent", undefined, {
          apiID: openrouterModel.api.id,
          apiPackage: openrouterModel.api.npm,
...
describe("session.message-v2.fromError", () => {
    ...
    cases.forEach((item) => {
      ...
      expect(result).toStrictEqual({
        ...
        data: {
          ...
          isRetryable: false,
          ...(item.code === "invalid_prompt" ? {} : { usageLimited: true }),
          responseBody: JSON.stringify(input),
        },
      })
    })
  ...
  test("classifies ZlibError from fetch as retryable APIError", () => {
    const zlibError = new Error(
      'ZlibError fetching "https://interbase.cloudflare.dev/anthropic/messages". For more information, pass `verbose: true` in the second argument to fetch()',
    )
