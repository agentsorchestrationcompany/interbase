...
const auth = { username: "interbase", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.INTERBASE_SERVER_PASSWORD
  else process.env.INTERBASE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.INTERBASE_SERVER_USERNAME
  else process.env.INTERBASE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener(backend: "effect-httpapi" | "hono" = "effect-httpapi") {
  Flag.INTERBASE_EXPERIMENTAL_HTTPAPI = backend === "effect-httpapi"
  Flag.INTERBASE_SERVER_PASSWORD = auth.password
  Flag.INTERBASE_SERVER_USERNAME = auth.username
  process.env.INTERBASE_SERVER_PASSWORD = auth.password
  process.env.INTERBASE_SERVER_USERNAME = auth.username
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-interbase-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-interbase-ticket": "1" }),
  ...
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-interbase-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  testPty("PTY connect token requires matching directory across mint and connect", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-interbase-ticket": "1" },
      })
