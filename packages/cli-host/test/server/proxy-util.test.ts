...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips interbase-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-interbase-directory": "/tmp/project",
          "x-interbase-workspace": "ws_123",
          "accept-encoding": "gzip",
          "x-custom": "keep",
        },
      })
      const result = ProxyUtil.headers(req)
      expect(result.get("x-interbase-directory")).toBeNull()
      expect(result.get("x-interbase-workspace")).toBeNull()
