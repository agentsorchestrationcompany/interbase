...
import { createOpencodeClient } from "@interbase/sdk/v2"
...
function app(backend: Backend, input?: { password?: string; username?: string }) {
  Flag.INTERBASE_EXPERIMENTAL_HTTPAPI = backend === "httpapi"
...
describe("HttpApi SDK", () => {
      ...
      Effect.gen(function* () {
        ...
        const bad = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("interbase", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client(backend, directory, {
            password: "secret",
            headers: { authorization: authorization("interbase", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
  ...
  httpapi(
    "returns BadRequest for unknown session model updates",
    withFakeLlm("httpapi", ({ sdk }) =>
      Effect.gen(function* () {
        const session = yield* capture(() => sdk.session.create({ title: "model update" }))
        const result = yield* capture(() =>
          sdk.session.update({
            sessionID: String(record(session.data).id),
            model: { providerID: "test", id: "missing-model" },
          }),
        )
