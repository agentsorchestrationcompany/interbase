...
const original = {
  OPENCODE_EXPERIMENTAL_HTTPAPI: Flag.INTERBASE_EXPERIMENTAL_HTTPAPI,
  OPENCODE_SERVER_PASSWORD: Flag.INTERBASE_SERVER_PASSWORD,
  OPENCODE_SERVER_USERNAME: Flag.INTERBASE_SERVER_USERNAME,
}
...
function app(input?: { password?: string; username?: string }) {
  Flag.INTERBASE_EXPERIMENTAL_HTTPAPI = true
  Flag.INTERBASE_SERVER_PASSWORD = input?.password
  Flag.INTERBASE_SERVER_USERNAME = input?.username
...
function requestBodyKey(spec: OpenApiSpec, body: unknown) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- Guarded above; test helper only needs this OpenAPI subset.
...
function requestBodySchemaKind(spec: OpenApiSpec, schema: OpenApiSchema | undefined) {
  if (!schema) return ""
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- `$ref` lookup is constrained to OpenAPI schema components in this test helper.
  const resolved = (
...
function responseContentTypes(input: {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion -- Guarded dynamic OpenAPI response lookup.
...
afterEach(async () => {
  Flag.INTERBASE_EXPERIMENTAL_HTTPAPI = original.OPENCODE_EXPERIMENTAL_HTTPAPI
  Flag.INTERBASE_SERVER_PASSWORD = original.OPENCODE_SERVER_PASSWORD
  Flag.INTERBASE_SERVER_USERNAME = original.OPENCODE_SERVER_USERNAME
...
describe("HttpApi server", () => {
  ...
  test("allows requests when auth is disabled", async () => {
    ...
    const response = await app().request(fileUrl(), {
      headers: {
        "x-interbase-directory": tmp.path,
      },
    })
  ...
  test("requires credentials when auth is enabled", async () => {
    ...
    const [missing, bad, good] = await Promise.all([
      app({ password: "secret" }).request(fileUrl(), {
        headers: { "x-interbase-directory": tmp.path },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("interbase", "wrong"),
          "x-interbase-directory": tmp.path,
        },
      }),
      app({ password: "secret" }).request(fileUrl(), {
        headers: {
          authorization: authorization("interbase", "secret"),
          "x-interbase-directory": tmp.path,
        },
      }),
    ])
  ...
  test("accepts auth_token query credentials", async () => {
    ...
    const response = await app({ password: "secret" }).request(
      fileUrl({ token: Buffer.from("interbase:secret").toString("base64") }),
      {
        headers: {
          "x-interbase-directory": tmp.path,
        },
  ...
  test("selects instance from query before directory header", async () => {
    ...
    const response = await app().request(fileUrl({ directory: query.path }), {
      headers: {
        "x-interbase-directory": header.path,
      },
    })
  ...
  test("serves global health from Effect HttpApi", async () => {
    const response = await app().request(`${GlobalPaths.health}?directory=/does/not/exist/interbase-test`)
