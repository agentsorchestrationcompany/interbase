...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under interbase.", () => {
    expect(paramToAttributeKey("name")).toBe("interbase.name")
    expect(paramToAttributeKey("slug")).toBe("interbase.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["interbase.sessionID"]).toBeUndefined()
    expect(attrs["interbase.messageID"]).toBeUndefined()
    expect(attrs["interbase.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under interbase. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["interbase.name"]).toBe("exa")
