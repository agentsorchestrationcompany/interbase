...
describe("experimental HttpApi", () => {
  test("serves read-only experimental endpoints through Hono bridge", async () => {
    ...
    const headers = { "x-interbase-directory": tmp.path }
    const [consoleState, toolList, toolIDs, worktrees, resources] = await Promise.all([
      app().request(ExperimentalPaths.console, { headers }),
      app().request(`${ExperimentalPaths.tool}?provider=interbase&model=gpt-5`, { headers }),
  ...
  testWorktreeMutations("serves worktree mutations through Hono bridge", async () => {
    ...
    const headers = { "x-interbase-directory": tmp.path, "content-type": "application/json" }
    ...
    expect(info).toMatchObject({ name: "api-test", branch: "interbase/api-test" })
