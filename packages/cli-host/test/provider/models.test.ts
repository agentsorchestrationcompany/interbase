...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.INTERBASE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.INTERBASE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.INTERBASE_MODELS_PATH = undefined
  Flag.INTERBASE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.INTERBASE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.INTERBASE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
beforeEach(async () => {
  mock.module("../../src/provider/models-snapshot.js", () => ({ snapshot: undefined }))
...
describe("ModelsDev Service", () => {
  ...
  it.live("get() prefers the bundled snapshot before disabled-fetch fallback", () =>
    Effect.gen(function* () {
      mock.module("../../src/provider/models-snapshot.js", () => ({ snapshot: fixture }))
      const state = yield* Ref.make(initialState)
      const result = yield* provided(
        state,
        ModelsDev.Service.use((s) => s.get()),
