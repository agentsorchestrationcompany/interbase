...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@interbase/acme")).toBe("@interbase/acme")
    expect(Npm.sanitize("@interbase/acme@1.0.0")).toBe("@interbase/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/interbase/acme.git"
    const expected = win ? "acme@git+https_//github.com/interbase/acme.git" : spec
